...
class LinkedList:
    ...
    def search_by_name(self, menu_name):
        if self.head is None:
            print("Linked list kosong.")
            return None
    ...
    def search_by_price(self, menu_price):
        if self.head is None:
            print("Linked list kosong.")
            return None
        ...
        current = self.head
        while current:
            if current.data.price == menu_price:
                return current.data
            current = current.next

        print(f"Menu dengan harga '{menu_price}' tidak ditemukan.")
        return None
...
class OrderSystem:
    def __init__(self):
        ...
        self.menu_list.append(Menu("Nasi Uduk", 17000))
        self.menu_list.append(Menu("Bakso", 15000))
        self.menu_list.append(Menu("Sate Ayam", 25000))
        self.menu_list.append(Menu("Martabak", 22000))
        self.menu_list.append(Menu("Capcay", 18000))
        self.menu_list.append(Menu("Es Teh", 5000))
    ...
    def search_menu(self):
        ...
        search_option = input("Masukkan pilihan pencarian: ")

        if search_option == '1':
            menu_name = input("Masukkan nama menu yang ingin dicari: ")
            found_menu = self.menu_list.search_by_name(menu_name)
            if found_menu:
                print(f"Menu '{menu_name}' ditemukan. Harga: Rp{found_menu.price}")
        elif search_option == '2':
            menu_price = int(input("Masukkan harga menu yang ingin dicari: "))
            found_menu = self.menu_list.search_by_price(menu_price)
            if found_menu:
                print(f"Menu dengan harga '{menu_price}' ditemukan. Nama: {found_menu.name}")
        else:
